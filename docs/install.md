...
| مشتری تأییدشده | `customer@ideban.test` | همان؛ پنل `/portal` |
| مشتری در انتظار تأیید | `pending@ideban.test` | همان؛ تا تأیید مدیر سفارش و پیام بسته است |
...
اگر این دیتابیس را قبلاً ساخته‌اید، فایل کامل را دوباره ایمپورت نکنید. دادهٔ شرکت، کاربر و حضور پاک می‌شود. برای پنل مشتری روی همان دیتابیس فقط این فایل را ایمپورت کنید:

`portal-upgrade.sql`

نسخهٔ داخل پروژه: `backend/database/sql/mysql/portal-upgrade.sql`

جدول‌های مشتری، کالا، سفارش، پیام و تیکت را اگر نباشند می‌سازد. مجوزها، نقش `client` و قابلیت `portal` را هم اضافه می‌کند. دوباره زدنش بی‌خطر است. حساب نمونه نمی‌سازد. بعد از آن، اگر دادهٔ نمونهٔ مشتری را می‌خواهید:

```bash
php artisan db:seed --class=Database\\Seeders\\DemoPortalSeeder
```

تقویم شمسی بدون تغییر اسکیما کار می‌کند. برای پر شدن گزارش کار نمونه، بعد از گرفتن کد:
